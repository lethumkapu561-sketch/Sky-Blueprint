...
function renderCVBuilder(el) {
  ...
  <div class="tool-screen">
    ...
    <p>Build a clean professional CV — download it as PDF and find matching jobs.</p>
    <div class="tab-bar">
    ...
    <div id="cvt-build">
      ...
      <!-- PERSONAL INFO -->
      <div class="cv-sec-title">Contact Information</div>
      <div class="form-row">
        <div class="form-group"><label>First Name *</label><input type="text" id="cv-fn" placeholder="e.g. Sipho"></div>
        <div class="form-group"><label>Last Name *</label><input type="text" id="cv-ln" placeholder="e.g. Dlamini"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Email Address</label><input type="email" id="cv-em" placeholder="sipho@email.com"></div>
        <div class="form-group"><label>Phone Number</label><input type="tel" id="cv-ph" placeholder="082 345 6789"></div>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
        ...
        <div>
          ...
          <p style="font-size:11px;color:var(--muted);margin:4px 0 0">JPG or PNG recommended</p>
        </div>
      </div>

      <!-- PERSONAL PROFILE -->
      <div class="cv-sec-title">Personal Profile / Summary (Optional)</div>
      <div class="form-group"><textarea id="cv-sum" rows="3" placeholder="A brief overview of your qualifications and career goals. Example: Dedicated sales professional with 3 years experience in retail and customer service across South Africa..."></textarea></div>
      ...
      <div class="form-group"><label>Field of Study / Qualification Name</label><input type="text" id="cv-qu" placeholder="e.g. BCom Business Management / N6 Electrical Engineering"></div>
      <div class="form-row">
        <div class="form-group"><label>Institution / School</label><input type="text" id="cv-inst" placeholder="e.g. University of Cape Town"></div>
        <div class="form-group"><label>Year Completed</label><input type="text" id="cv-year" placeholder="e.g. 2022"></div>
      </div>
      <div class="form-group"><label>Honours / Awards / Relevant Courses (Optional)</label><input type="text" id="cv-honors" placeholder="e.g. Dean List 2021, cum laude, Accounting III"></div>
      ...
      <div class="form-group">
        ...
        <select id="cv-exp">
          <option value="0">No experience (Fresher / Student)</option>
      ...
      <div class="form-row">
        <div class="form-group"><label>Job Title</label><input type="text" id="cv-jt" placeholder="e.g. Sales Manager"></div>
        <div class="form-group"><label>Company / Organization</label><input type="text" id="cv-co" placeholder="e.g. Vodacom SA"></div>
      </div>
      <div class="form-row">
      ...
      <div class="form-group"><label>Job Responsibilities (one per line)</label>
        <textarea id="cv-resp" rows="4" placeholder="Managed a team of 5 sales representatives across Western Cape&#10;Achieved 120% of monthly sales targets consistently&#10;Trained new staff on product knowledge and customer service&#10;Prepared weekly sales reports for management"></textarea>
      </div>

      <!-- SKILLS -->
      <div class="cv-sec-title">Skills</div>
      <div class="form-group"><input type="text" id="cv-sk" placeholder="e.g. Microsoft Office, Customer Service, Driving Licence, Python, Welding, English & Zulu"></div>

      <!-- OPTIONAL SECTIONS -->
      <div class="cv-sec-title">Additional Information (Optional)</div>
      <div class="form-group"><label>Certifications / Licences</label><input type="text" id="cv-cert" placeholder="e.g. Driver Licence Code 10, First Aid Level 1"></div>
      <div class="form-group"><label>Awards & Honours</label><input type="text" id="cv-awards" placeholder="e.g. Employee of the Year 2023 — Shoprite Holdings"></div>
      <div class="form-group"><label>Languages</label><input type="text" id="cv-langs" placeholder="e.g. English (Fluent), Zulu (Native), Afrikaans (Basic)"></div>
      <div class="form-group"><label>References</label>
        <select id="cv-refs">
          <option value="available">Available on request</option>
          <option value="listed">I will list my references</option>
        </select>
      </div>

      <button class="btn-primary" style="width:100%;box-sizing:border-box;margin-top:8px;font-size:16px;padding:16px" onclick="buildAndMatchCV()">🚀 Build My Professional CV</button>
      <div id="cv-msg" style="margin-top:16px"></div>
    </div>
    ...
    <div id="cvt-jobs" style="display:none">
      <div id="job-match-content">
        <div style="text-align:center;padding:40px;color:var(--muted)">
          <div style="font-size:48px;margin-bottom:12px">📄</div>
          <p>Build your CV first to see matching jobs</p>
        </div>
      </div>
    </div>

    <div id="cvt-upload" style="display:none">
      <p style="font-size:13px;color:var(--muted);margin-bottom:20px">Upload your existing CV — AI will read it and find matching jobs for your level.</p>
      <div style="border:2px dashed rgba(56,189,248,0.3);border-radius:14px;padding:32px;text-align:center;margin-bottom:20px">
        ...
        <p style="color:var(--muted);font-size:13px;margin-bottom:16px">PDF, Word (.docx) or Text file</p>
...
async function buildAndMatchCV() {
  ...
  var resp = (document.getElementById('cv-resp')       || {value:''}).value.trim();
  var qu   = (document.getElementById('cv-qu')         || {value:''}).value.trim();
  var inst = (document.getElementById('cv-inst')       || {value:''}).value.trim();
  var yr   = (document.getElementById('cv-year')       || {value:''}).value.trim();
  var hon  = (document.getElementById('cv-honors')     || {value:''}).value.trim();
  var sk   = (document.getElementById('cv-sk')         || {value:''}).value.trim();
  var sum  = (document.getElementById('cv-sum')        || {value:''}).value.trim();
  var cert = (document.getElementById('cv-cert')       || {value:''}).value.trim();
  var awds = (document.getElementById('cv-awards')     || {value:''}).value.trim();
  var lang = (document.getElementById('cv-langs')      || {value:''}).value.trim();
  ...
  document.getElementById('cv-msg').innerHTML =
    '<div style="text-align:center;padding:20px;color:var(--muted)">⏳ Building your professional CV...</div>';
  ...
  var respLines = resp ? resp.split('\n').filter(function(l){ return l.trim(); }) : [];
  var skillArr  = sk ? sk.split(',').map(function(s){ return s.trim(); }).filter(Boolean) : [];

  var cvHTML =
'<!DOCTYPE html><html lang="en"><head>' +
'<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
'<title>' + fn + ' ' + ln + ' \u2014 CV</title>' +
'<style>' +
'@import url(\'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Syne:wght@700;800&display=swap\');' +
'*{margin:0;padding:0;box-sizing:border-box;}' +
'html,body{background:#060914;min-height:100vh;}' +
'body{font-family:\'Inter\',Arial,sans-serif;color:#e2e8f0;font-size:10.5pt;line-height:1.6;}' +
'.page{max-width:820px;margin:0 auto;background:#060914;position:relative;}' +

/* SIDEBAR */
'.sidebar{position:absolute;top:0;left:0;width:220px;min-height:100%;background:linear-gradient(180deg,#0d1f3c 0%,#1a1040 100%);padding:32px 20px;border-right:1px solid rgba(56,189,248,0.15);}' +
'.main{margin-left:220px;padding:32px 32px 32px 28px;min-height:100vh;}' +

/* PHOTO */
'.photo-wrap{text-align:center;margin-bottom:20px;}' +
'.photo-img{width:100px;height:100px;border-radius:50%;border:3px solid #38bdf8;object-fit:cover;display:block;margin:0 auto;}' +
'.avatar{width:100px;height:100px;border-radius:50%;border:3px solid #38bdf8;background:#1e3a5f;display:flex;align-items:center;justify-content:center;font-size:40px;margin:0 auto;}' +

/* NAME in sidebar */
'.cv-name{font-family:\'Syne\',Arial,sans-serif;font-size:16pt;font-weight:800;color:#fff;text-align:center;margin-bottom:4px;line-height:1.2;}' +
'.cv-jobtitle{font-size:9.5pt;color:#38bdf8;text-align:center;margin-bottom:20px;font-weight:500;}' +

/* SIDEBAR SECTIONS */
'.sb-sec-title{font-size:8pt;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:#38bdf8;margin:18px 0 10px;padding-bottom:4px;border-bottom:1px solid rgba(56,189,248,0.3);}' +
'.sb-item{font-size:9.5pt;color:#b0c4d8;margin-bottom:6px;display:flex;align-items:flex-start;gap:6px;line-height:1.5;}' +
'.sb-icon{color:#38bdf8;flex-shrink:0;font-size:10pt;}' +
'.skill-bar-wrap{margin-bottom:8px;}' +
'.skill-name{font-size:9pt;color:#cbd5e1;margin-bottom:3px;}' +
'.skill-bar{height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden;}' +
'.skill-fill{height:100%;background:linear-gradient(90deg,#38bdf8,#6366f1);border-radius:2px;}' +

/* MAIN SECTIONS */
'.sec-title{font-size:10pt;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#38bdf8;margin:0 0 12px;padding-bottom:5px;border-bottom:2px solid rgba(56,189,248,0.4);}' +
'.section{margin-bottom:24px;}' +

/* EDUCATION */
'.edu-block{margin-bottom:10px;}' +
'.edu-qual{font-size:11pt;font-weight:700;color:#fff;}' +
'.edu-field{font-size:10pt;color:#94a3b8;margin-top:2px;}' +
'.edu-meta{display:flex;justify-content:space-between;margin-top:3px;}' +
'.edu-inst{font-size:9.5pt;color:#64748b;}' +
'.edu-yr{font-size:9.5pt;color:#64748b;white-space:nowrap;}' +
'.edu-hon{font-size:9.5pt;color:#38bdf8;font-style:italic;margin-top:2px;}' +

/* EXPERIENCE */
'.exp-block{margin-bottom:16px;padding-left:12px;border-left:2px solid rgba(56,189,248,0.3);}' +
'.exp-header{display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:4px;margin-bottom:2px;}' +
'.exp-title{font-size:11pt;font-weight:700;color:#fff;}' +
'.exp-dates{font-size:9pt;color:#64748b;white-space:nowrap;}' +
'.exp-company{font-size:10pt;color:#38bdf8;font-weight:600;margin-bottom:6px;}' +
'.exp-ul{padding-left:16px;margin:0;}' +
'.exp-ul li{font-size:10pt;color:#94a3b8;margin-bottom:4px;line-height:1.6;}' +

/* SUMMARY */
'.summary-text{font-size:10.5pt;color:#94a3b8;line-height:1.7;}' +

/* EXTRAS */
'.extra-ul{padding-left:16px;margin:0;}' +
'.extra-ul li{font-size:10pt;color:#94a3b8;margin-bottom:4px;}' +

/* REFS */
'.ref-text{font-size:10pt;color:#64748b;font-style:italic;}' +

/* FOOTER */
'.cv-footer{margin-top:24px;padding-top:12px;border-top:1px solid rgba(56,189,248,0.2);display:flex;justify-content:space-between;align-items:center;}' +
'.footer-brand{font-size:8.5pt;color:#38bdf8;font-weight:600;}' +
'.footer-note{font-size:8pt;color:#334155;}' +

/* PRINT */
'@media print{' +
'html,body{background:#060914!important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}' +
'.page{max-width:100%;}' +
'.sidebar{position:fixed!important;top:0;left:0;height:100%;}' +
'.no-print{display:none!important;}' +
'}' +
'@media screen and (max-width:600px){' +
'.sidebar{position:relative;width:100%;min-height:auto;border-right:none;border-bottom:1px solid rgba(56,189,248,0.15);}' +
...
(photo ? '<img src="' + photo + '" class="photo-img" alt="Profile">' : '<div class="avatar">👤</div>') +
'</div>' +

/* Name */
'<div class="cv-name">' + fn + ' ' + ln + '</div>' +
'<div class="cv-jobtitle">' + (jt || qualLabel) + '</div>' +

/* Contact */
'<div class="sb-sec-title">Contact</div>' +
(ph ? '<div class="sb-item"><span class="sb-icon">📞</span><span>' + ph + '</span></div>' : '') +
(em ? '<div class="sb-item"><span class="sb-icon">✉</span><span>' + em + '</span></div>' : '') +
(ci ? '<div class="sb-item"><span class="sb-icon">📍</span><span>' + ci + '</span></div>' : '') +

/* Skills in sidebar as bars */
(skillArr.length > 0 ?
  '<div class="sb-sec-title">Skills</div>' +
  skillArr.slice(0, 8).map(function(s, i) {
    var pct = Math.max(60, 100 - i * 5);
    return '<div class="skill-bar-wrap"><div class="skill-name">' + s + '</div>' +
...
(lang ?
  '<div class="sb-sec-title">Languages</div>' +
  lang.split(',').map(function(l){ return '<div class="sb-item"><span class="sb-icon">🌐</span><span>' + l.trim() + '</span></div>'; }).join('')
: '') +

/* References in sidebar */
'<div class="sb-sec-title">References</div>' +
'<div class="sb-item" style="font-style:italic">Available on request</div>' +
...
/* PERSONAL PROFILE */
(sum ?
  '<div class="section"><div class="sec-title">Personal Profile</div><div class="summary-text">' + sum + '</div></div>'
: '') +
...
(qu ? '<div class="edu-field">' + qu + '</div>' : '') +
'<div class="edu-meta">' +
(inst ? '<span class="edu-inst">' + inst + '</span>' : '<span></span>') +
(yr ? '<span class="edu-yr">Graduated: ' + yr + '</span>' : '') +
'</div>' +
(hon ? '<div class="edu-hon">' + hon + '</div>' : '') +
'</div></div>' +

/* EXPERIENCE */
((jt || co || resp) ?
  '<div class="section"><div class="sec-title">Experience</div>' +
  '<div class="exp-block">' +
  '<div class="exp-header"><div class="exp-title">' + (jt || '') + '</div>' +
  '<div class="exp-dates">' + [sd, ed].filter(Boolean).join(' \u2013 ') + '</div></div>' +
  (co ? '<div class="exp-company">' + co + '</div>' : '') +
  (respLines.length > 0 ?
    '<ul class="exp-ul">' + respLines.map(function(l){ return '<li>' + l + '</li>'; }).join('') + '</ul>'
  : '') +
  '</div></div>'
: '') +
...
/* CERTIFICATIONS */
(cert ?
  '<div class="section"><div class="sec-title">Certifications &amp; Licences</div>' +
  '<ul class="extra-ul">' + cert.split(',').map(function(c){ return '<li>' + c.trim() + '</li>'; }).join('') + '</ul></div>'
: '') +

/* AWARDS */
(awds ?
  '<div class="section"><div class="sec-title">Awards &amp; Honours</div>' +
  '<ul class="extra-ul">' + awds.split(',').map(function(a){ return '<li>' + a.trim() + '</li>'; }).join('') + '</ul></div>'
: '') +

/* FOOTER */
'<div class="cv-footer">' +
'<div class="footer-brand">Sky Blueprint</div>' +
'<div class="footer-note">Your Digital Life, Unified \u2014 skyblueprint.co.za</div>' +
'</div>' +
...
'<button onclick="window.print()" style="background:linear-gradient(135deg,#38bdf8,#6366f1);color:#fff;border:none;border-radius:10px;padding:14px 36px;font-size:14px;font-weight:700;cursor:pointer;font-family:Inter,Arial,sans-serif;margin-right:10px">📥 Download as PDF</button>' +
'<button onclick="window.close()" style="background:rgba(255,255,255,0.05);color:#94a3b8;border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:14px 24px;font-size:14px;cursor:pointer;font-family:Inter,Arial,sans-serif">Close</button>' +
'<p style="color:#475569;font-size:11px;margin-top:10px">Press Download as PDF \u2192 In print dialog choose \u201CSave as PDF\u201D</p>' +
'</div>' +
...
'</body></html>';
  ...
  document.getElementById('cv-msg').innerHTML =
    '<div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.2);border-radius:14px;padding:24px">' +
    '<strong style="color:var(--green);font-size:16px;display:block;margin-bottom:6px">CV Built for ' + fn + ' ' + ln + '!</strong>' +
    '<p style="color:var(--muted);font-size:13px;margin-bottom:16px">Sky Blueprint branded, professional, ready to download as PDF on mobile and desktop.</p>' +
    '<div style="display:flex;gap:10px;flex-wrap:wrap">' +
    '<button class="btn-primary" style="flex:1;min-width:140px;box-sizing:border-box;font-size:14px" onclick="downloadCV()">📥 Download CV as PDF</button>' +
    '<button style="flex:1;min-width:140px;box-sizing:border-box;background:rgba(56,189,248,0.1);border:1px solid rgba(56,189,248,0.3);color:#38bdf8;border-radius:10px;padding:12px;font-family:var(--font);cursor:pointer;font-weight:600;font-size:14px" onclick="previewCV()">👁 Preview CV</button>' +
    '</div></div>';
