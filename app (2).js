...
async function buildAndMatchCV() {
  ...
  var honors = (document.getElementById('cv-honors') || {value:''}).value.trim();
  ...
  if (!fn || !qual) {
    alert('Please enter your name and select your highest qualification to continue.');
    return;
  }

  document.getElementById('cv-msg').innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted)"><div style="font-size:32px;margin-bottom:8px">⏳</div>Building your professional CV...</div>';

  // Build responsibilities bullet points
  var respLines = resp ? resp.split('\n').filter(function(l){ return l.trim(); }) : [];
  var respHTML = respLines.map(function(l){ return '<li style="margin-bottom:6px;font-size:13px;color:#444;line-height:1.6">' + l.trim() + '</li>'; }).join('');

  // Build skills
  var skillsArr = sk ? sk.split(',').map(function(s){ return s.trim(); }).filter(function(s){ return s; }) : [];
  var skillsHTML = skillsArr.map(function(s){ return '<span style="background:#eef6ff;color:#0d4f8a;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;display:inline-block;margin:3px;">' + s + '</span>'; }).join('');

  // Qualification label
  var qualLabels = {
    grade9:'Grade 9', grade10:'Grade 10', grade11:'Grade 11',
    matric:'Grade 12 / Matric', n4:'N4 Certificate', n5:'N5 Certificate',
    n6:'N6 Certificate', diploma:'Diploma', degree:'Degree',
    honours:'Honours Degree', masters:'Masters Degree', phd:'PhD / Doctorate'
  };
  var qualLabel = qualLabels[qual] || qual;

  // Build full professional CV
  var cvHTML = '<!DOCTYPE html><html lang="en"><head>' +
    '<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    '<title>' + fn + ' ' + ln + ' — CV</title>' +
    '<style>' +
    '* { margin:0; padding:0; box-sizing:border-box; }' +
    'body { font-family: Arial, Helvetica, sans-serif; font-size: 11pt; color: #1a1a2e; background: #fff; line-height: 1.5; }' +
    '.page { max-width: 850px; margin: 0 auto; padding: 0; }' +
    '.header { background: linear-gradient(135deg, #0a1628, #1a1040); color: #fff; padding: 32px 40px; display: flex; align-items: center; gap: 24px; }' +
    '.photo { width: 88px; height: 88px; border-radius: 50%; border: 3px solid #38bdf8; object-fit: cover; flex-shrink: 0; }' +
    '.avatar { width: 88px; height: 88px; border-radius: 50%; border: 3px solid #38bdf8; background: #1e3a5f; display: flex; align-items: center; justify-content: center; font-size: 32px; flex-shrink: 0; color:#fff; }' +
    '.hinfo { flex: 1; }' +
    '.hname { font-size: 24pt; font-weight: 700; letter-spacing: -0.5px; margin-bottom: 4px; }' +
    '.htitle { font-size: 13pt; color: #38bdf8; margin-bottom: 10px; font-weight: 500; }' +
    '.hcontact { display: flex; flex-wrap: wrap; gap: 16px; font-size: 10pt; color: #b0c4d8; }' +
    '.hcontact span { display: flex; align-items: center; gap: 5px; }' +
    '.body { padding: 32px 40px; }' +
    '.section { margin-bottom: 26px; page-break-inside: avoid; }' +
    '.sec-title { font-size: 10pt; font-weight: 700; color: #38bdf8; text-transform: uppercase; letter-spacing: 2px; border-bottom: 2px solid #38bdf8; padding-bottom: 5px; margin-bottom: 14px; }' +
    '.summary-text { font-size: 11pt; color: #333; line-height: 1.7; }' +
    '.edu-block { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px; }' +
    '.edu-left .edu-qual { font-size: 12pt; font-weight: 700; color: #1a1a2e; }' +
    '.edu-left .edu-field { font-size: 11pt; color: #444; margin-top: 2px; }' +
    '.edu-left .edu-honors { font-size: 10pt; color: #666; font-style: italic; margin-top: 3px; }' +
    '.edu-right { text-align: right; }' +
    '.edu-right .edu-school { font-size: 11pt; color: #555; }' +
    '.edu-right .edu-year { font-size: 10pt; color: #888; margin-top: 2px; }' +
    '.exp-title { font-size: 12pt; font-weight: 700; color: #1a1a2e; }' +
    '.exp-company { font-size: 11pt; color: #38bdf8; font-weight: 600; }' +
    '.exp-dates { font-size: 10pt; color: #888; margin: 2px 0 8px; }' +
    '.exp-list { padding-left: 18px; margin-top: 6px; }' +
    '.skills-wrap { display: flex; flex-wrap: wrap; gap: 0; }' +
    '.skill-tag { background: #eef6ff; color: #0d4f8a; padding: 4px 12px; border-radius: 20px; font-size: 10pt; font-weight: 600; display: inline-block; margin: 3px; }' +
    '.extra-item { font-size: 11pt; color: #333; margin-bottom: 6px; padding-left: 14px; position: relative; }' +
    '.extra-item:before { content: "•"; position: absolute; left: 0; color: #38bdf8; font-weight: 700; }' +
    '.footer { border-top: 1px solid #e0e8f0; padding: 14px 40px; text-align: center; font-size: 9pt; color: #aaa; background: #f9fafc; }' +
    '@media print { body { margin:0; } .no-print { display:none!important; } .page { max-width:100%; } }' +
    '</style></head><body>' +
    '<div class="page">' +

    // HEADER
    '<div class="header">' +
    (photo ? '<img src="' + photo + '" class="photo" alt="Profile Photo">' : '<div class="avatar">👤</div>') +
    '<div class="hinfo">' +
    '<div class="hname">' + fn + ' ' + ln + '</div>' +
    '<div class="htitle">' + (jt || qualLabel) + '</div>' +
    '<div class="hcontact">' +
    (em ? '<span>✉ ' + em + '</span>' : '') +
    (ph ? '<span>📞 ' + ph + '</span>' : '') +
    (ci ? '<span>📍 ' + ci + '</span>' : '') +
    ...
    // PERSONAL SUMMARY
    (sum ? '<div class="section"><div class="sec-title">Personal Profile</div><div class="summary-text">' + sum + '</div></div>' : '') +
    ...
    (honors ? '<div class="edu-honors">' + honors + '</div>' : '') +
    '</div>' +
    '<div class="edu-right">' +
    (inst ? '<div class="edu-school">' + inst + '</div>' : '') +
    (yr ? '<div class="edu-year">Completed: ' + yr + '</div>' : '') +
    '</div></div></div>' +

    // SKILLS
    (sk ? '<div class="section"><div class="sec-title">Skills</div><div class="skills-wrap">' + skillsHTML + '</div></div>' : '') +

    // EXPERIENCE
    ((jt || co || resp) ? '<div class="section"><div class="sec-title">Experience</div>' +
    '<div class="exp-title">' + (jt || 'Role') + '</div>' +
    (co ? '<div class="exp-company">' + co + '</div>' : '') +
    ((sd || ed) ? '<div class="exp-dates">' + (sd || '') + (sd && ed ? ' – ' : '') + (ed || '') + '</div>' : '') +
    (respLines.length > 0 ? '<ul class="exp-list">' + respHTML + '</ul>' : '') +
    '</div>' : '') +

    // CERTIFICATIONS
    (cert ? '<div class="section"><div class="sec-title">Certifications & Licences</div>' +
    cert.split(',').map(function(c){ return '<div class="extra-item">' + c.trim() + '</div>'; }).join('') +
    '</div>' : '') +

    // AWARDS
    (awards ? '<div class="section"><div class="sec-title">Awards & Honours</div>' +
    awards.split(',').map(function(a){ return '<div class="extra-item">' + a.trim() + '</div>'; }).join('') +
    '</div>' : '') +

    // LANGUAGES
    (langs ? '<div class="section"><div class="sec-title">Languages</div>' +
    langs.split(',').map(function(l){ return '<div class="extra-item">' + l.trim() + '</div>'; }).join('') +
    '</div>' : '') +

    // REFERENCES
    '<div class="section"><div class="sec-title">References</div><div style="font-size:11pt;color:#555;font-style:italic">Available on request</div></div>' +

    '</div>' + // end body
    '<div class="footer">Created with Sky Blueprint — Your Digital Life, Unified | lethumkapu561@gmail.com | 065 601 3544</div>' +
    '</div></body></html>';
  ...
  document.getElementById('cv-msg').innerHTML =
    ...
    '<strong style="color:var(--green);font-size:16px;display:block;margin-bottom:6px">✅ Professional CV Built for ' + fn + ' ' + ln + '!</strong>' +
    '<p style="color:var(--muted);font-size:13px;margin-bottom:16px">Your CV includes: Contact info, Education, Skills, Experience' +
    (cert ? ', Certifications' : '') + (awards ? ', Awards' : '') + (langs ? ', Languages' : '') + ', References.</p>' +
